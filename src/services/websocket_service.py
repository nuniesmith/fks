    ...
    def store_price_update(self, symbol: str, data: Dict):
        ...
        try:
            price_data = {
                ...
                'timestamp': datetime.now(TIMEZONE).isoformat(),
                'event_time': datetime.fromtimestamp(int(data.get('E', 0)) / 1000, tz=TIMEZONE).isoformat()
            }
            ...
            self.redis.zadd(
                'live_prices:all',
                {symbol: datetime.now(TIMEZONE).timestamp()}
            )
            
            logger.debug(f"📊 {symbol}: ${price_data['price']:,.2f} "
...
class PriceAggregator:
    ...
    def calculate_market_index(self, base_prices: Optional[Dict[str, float]] = None) -> Optional[float]:
        """
