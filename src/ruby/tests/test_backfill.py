...
def _make_bars_df(
    ...
    idx = pd.date_range(start=start_time, periods=n, freq=freq, tz="UTC")
...
def _use_sqlite_tempdb(tmp_path, monkeypatch):
    ...
    # Also reset models and db_layer if imported — must update both because
    # _get_sqlite_conn() reads os.getenv("DB_PATH", db_layer.DB_PATH) and the
    # monkeypatch env var handles the os.getenv() side, while the direct
    # attribute assignments reset any cached SQLAlchemy engine state.
    try:
