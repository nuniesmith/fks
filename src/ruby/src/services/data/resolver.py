...
Usage
-----
    from services.data.resolver import DataResolver
...
def _try_kraken_api(symbol: str, days: int) -> pd.DataFrame | None:
    ...
    try:
        from integrations.kraken_client import KrakenDataProvider
...
def _backfill_postgres(symbol: str, df: pd.DataFrame) -> bool:
    ...
    """
    try:
        from services.engine.backfill import _get_conn, _store_bars  # type: ignore[private-usage]
...
def _try_rithmic_l1(symbol: str) -> pd.DataFrame | None:
    ...
    try:
        ...
        from integrations.rithmic_client import get_stream_manager
