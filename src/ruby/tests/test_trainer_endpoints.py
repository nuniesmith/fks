...
# The stub must expose attributes that downstream libraries (scipy, sklearn,
# model classes) probe at import time — otherwise ``AttributeError: module
# 'torch' has no attribute 'Tensor'`` crashes every test that runs after
# this file in the same process.
_torch_stub = types.ModuleType("torch")
_torch_stub.Tensor = MagicMock  # type: ignore[attr-defined]
...
_torch_stub.manual_seed = MagicMock()  # type: ignore[attr-defined]
_torch_stub.no_grad = MagicMock(return_value=MagicMock(__enter__=MagicMock(), __exit__=MagicMock()))  # type: ignore[attr-defined]
_torch_stub.tensor = MagicMock()  # type: ignore[attr-defined]
_torch_stub.float32 = "float32"  # type: ignore[attr-defined]

# torch.nn sub-module stub
_nn_stub = types.ModuleType("torch.nn")
_nn_stub.Module = type("Module", (), {"__init__": lambda self: None})  # type: ignore[attr-defined]
_nn_stub.LSTM = MagicMock  # type: ignore[attr-defined]
_nn_stub.Linear = MagicMock  # type: ignore[attr-defined]
_nn_stub.MSELoss = MagicMock  # type: ignore[attr-defined]
_torch_stub.nn = _nn_stub  # type: ignore[attr-defined]

# torch.optim sub-module stub
_optim_stub = types.ModuleType("torch.optim")
_optim_stub.Adam = MagicMock  # type: ignore[attr-defined]
_torch_stub.optim = _optim_stub  # type: ignore[attr-defined]

# torch.utils.data stub
_data_stub = types.ModuleType("torch.utils.data")
_data_stub.DataLoader = MagicMock  # type: ignore[attr-defined]
_data_stub.TensorDataset = MagicMock  # type: ignore[attr-defined]
...
if "torch" not in sys.modules:
    ...
    sys.modules.setdefault("torch.optim", _optim_stub)
    sys.modules.setdefault("torch.utils", types.ModuleType("torch.utils"))
    sys.modules.setdefault("torch.utils.data", _data_stub)
