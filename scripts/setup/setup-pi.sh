...
#   6. Installs Python 3.13+ and pip (for local dev/testing)
...
# ---------------------------------------------------------------------------
step_python() {
    header "Step 6/9 — Python 3.13+"

    # Check if a suitable Python is already installed
    local py=""
    for candidate in python3.13 python3.13; do
    ...
    if [ -n "$py" ]; then
        ...
        # Check if default python3 is >= 3.13
    ...
    local final_py
    for candidate in python3.13 python3.13 python3; do
        if command -v "$candidate" &>/dev/null; then
            final_py="$candidate"
            break
        fi
