...
Usage::
    ...
    from services.data.sync import DataSyncService, sync_router
...
# ---------------------------------------------------------------------------
# TradingView symbol mapping — maps internal FKS data tickers to TV format
# ---------------------------------------------------------------------------
# These are the canonical TV symbols for our focus instruments.
# Format: "EXCHANGE:ROOT_SYMBOL1!" for front-month futures.
_TV_SYMBOL_MAP: dict[str, str] = {
    # CME Micro futures
    "MGC=F": "COMEX:MGC1!",
    "SIL=F": "COMEX:SIL1!",
    "MES=F": "CME_MICRO:MES1!",
    "MNQ=F": "CME_MICRO:MNQ1!",
    "M2K=F": "CME_MICRO:M2K1!",
    "MYM=F": "CBOT:MYM1!",
    "ZN=F": "CBOT:ZN1!",
    "ZB=F": "CBOT:ZB1!",
    "ZW=F": "CBOT:ZW1!",
    # Also handle yfinance-style older tickers
    "SI=F": "COMEX:SIL1!",
    "GC=F": "COMEX:GC1!",
    "ES=F": "CME_MINI:ES1!",
    "NQ=F": "CME_MINI:NQ1!",
    "RTY=F": "CME_MICRO:M2K1!",
    "YM=F": "CBOT:MYM1!",
    # Kraken crypto → Coinbase/Binance TV equivalent for broader coverage
...
def _get_sync_symbols() -> list[str]:
    ...
    try:
        from core.models import ASSETS, CRYPTO_TICKERS
    ...
    try:
        from core.models import ENABLE_KRAKEN_CRYPTO, KRAKEN_CONTRACT_SPECS
...
def _get_conn():
    ...
    try:
        from core.models import _get_conn as models_get_conn
...
def _is_postgres() -> bool:
    ...
    try:
        from core.models import _is_using_postgres
...
class DataSyncService:
    ...
    def __init__(self) -> None:
        ...
        self._tv_task: asyncio.Task | None = None
    ...
    async def run(self) -> None:
        ...
        # Start TradingView real-time feed as a parallel background task.
        # It runs independently of the historical sync loop — the two work
        # side by side: TV keeps Redis ticks fresh, sync keeps Postgres bars fresh.
        self._tv_task = asyncio.create_task(self._run_tv_feed())
    ...
    async def _run_tv_feed(self) -> None:
        """Background task: maintain TradingView real-time subscriptions.

        When ``TV_SESSION_ID`` is set in the environment, this coroutine
        connects the TradingView WebSocket client and subscribes to all
        tracked symbols.  The TV client publishes incoming ticks to Redis
        ``tv:ticks:{tv_symbol}`` for consumption by SSE and chart endpoints.

        The method runs until ``stop()`` is called (``self._running`` becomes
        False).  On connection failure it backs off exponentially (up to 5
        minutes) before retrying.  Symbols are re-subscribed automatically
        after reconnects.
        """
        import os
        ...
        if not session_id:
            logger.debug("TV_SESSION_ID not set — TradingView feed disabled")
            return

        logger.info("TradingView real-time feed starting (session_id=set)")

        # Build list of TV symbols from current sync symbols
        def _get_tv_symbols() -> list[str]:
            tv_symbols: list[str] = []
            for internal_sym in _get_sync_symbols():
                tv_sym = _TV_SYMBOL_MAP.get(internal_sym)
                if tv_sym:
                    tv_symbols.append(tv_sym)
            return tv_symbols

        backoff = 5  # seconds; grows up to 300
        while self._running:
            try:
                from integrations.tradingview_client import (  # type: ignore[import-unresolved]
                    TradingViewError,
                    get_tv_client,
                )

                client = get_tv_client(session_id=session_id)
                await client.connect()
                self._tv_connected = True
                logger.info("TradingView WebSocket connected")

                tv_symbols = _get_tv_symbols()
                for tv_sym in tv_symbols:
                    try:
                        await client._create_subscription(tv_sym)
                        logger.debug("TV subscribed", symbol=tv_sym)  # type: ignore[call-arg]
                    except Exception as sub_exc:
                        logger.warning(  # type: ignore[call-arg]
                            "TV subscribe failed (non-fatal)",
                            symbol=tv_sym,
                            error=str(sub_exc),
                        )

                logger.info(
                    "TradingView: %d symbols subscribed, ticks publishing to Redis tv:ticks:*",
                    len(tv_symbols),
                )

                # Keep alive — check every 30 s and resubscribe dropped sessions
                while self._running and client.is_connected:
                    await asyncio.sleep(30)
                    current_subs = client.active_subscriptions
                    for tv_sym in tv_symbols:
                        if tv_sym not in current_subs:
                            try:
                                await client._create_subscription(tv_sym)
                                logger.info("TV resubscribed", symbol=tv_sym)  # type: ignore[call-arg]
                            except Exception:
                                pass

                if self._running:
                    logger.warning("TradingView WebSocket disconnected — will reconnect")
                    self._tv_connected = False

            except TradingViewError as exc:
                self._tv_connected = False
                logger.warning(
                    "TradingView feed error (will retry in %ds): %s",
                    backoff,
                    exc,
                )
            except asyncio.CancelledError:
                logger.info("TradingView feed task cancelled")
                break
            except Exception as exc:
                self._tv_connected = False
                logger.warning(
                    "TradingView feed unexpected error (will retry in %ds): %s",
                    backoff,
                    exc,
                    exc_info=True,
                )
        ...
        self._tv_connected = False
        logger.info("TradingView real-time feed stopped")
