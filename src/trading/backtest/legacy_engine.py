...
def run_backtest(df_prices, M, atr_period=14, sl_multiplier=2, tp_multiplier=3):
    ...
    for i in range(1, len(signal)):
        ...
        if prev_signal == 0 and curr_signal == 1:  # Enter
            ...
            trades.append({'date': date, 'action': 'ENTER', 'symbols': SYMBOLS, 'prices': [closes[base_sym].iloc[i] for base_sym in closes.columns], 'sl': sl, 'tp': tp})
            ...
            position = 0
            trades.append({'date': date, 'action': 'EXIT', 'symbols': SYMBOLS, 'prices': [closes[base_sym].iloc[i] for base_sym in closes.columns]})
