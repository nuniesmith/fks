...
Public API::
    ...
    from analysis.cross_asset import (
...
# Active training symbols: MGC, SIL, MES, MNQ, M2K, MYM
# ZN (10Y T-Note) removed 2026-03 — rate-driven / inverse-correlation with
# equities; per-asset 66.5%, per-group 73.4% — below signal threshold.
# All peer references are restricted to active symbols so the dataset generator
# never pre-loads dropped symbols during training.
PEER_MAP: dict[str, dict[str, Any]] = {
    # ── Metals ────────────────────────────────────────────────────────────
    "MGC": {
        "asset_name": "Gold",
        "primary_peer": "SIL",  # Gold↔Silver: tightest metals correlation
        "cross_class_peers": ["MES", "MYM"],  # Gold↔S&P (risk-off), Gold↔Dow (broad risk appetite)
    },
    "SIL": {
        "asset_name": "Silver",
    ...
    "MES": {
        ...
        "cross_class_peers": ["MGC", "MYM"],  # S&P↔Gold (risk-off hedge), S&P↔Dow (index confirm)
    },
    "MNQ": {
        "asset_name": "Nasdaq",
        "primary_peer": "MES",  # Nasdaq↔S&P: primary index peer
        "cross_class_peers": ["MGC", "MES"],  # NQ↔Gold (risk-off), NQ↔S&P (index confirm)
    },
    "M2K": {
        "asset_name": "Russell 2000",
        "primary_peer": "MES",  # Russell↔S&P: broad market correlation
        "cross_class_peers": ["MNQ", "MGC"],  # Russell↔Nasdaq, Russell↔Gold (risk appetite)
    },
    "MYM": {
        "asset_name": "Dow Jones",
        "primary_peer": "MES",  # Dow↔S&P: near-identical index correlation
        "cross_class_peers": ["MNQ", "SIL"],  # Dow↔Nasdaq, Dow↔Silver (commodity/risk)
    },
    # ── Agricultural (ZN/ZB removed 2026-03) ─────────────────────────────
    "ZW": {
        "asset_name": "Wheat",
        "primary_peer": "MGC",  # Wheat↔Gold: commodity complex (inflation/USD)
        "cross_class_peers": ["MES", "SIL"],  # Wheat↔S&P (risk appetite), Wheat↔Silver (commodity)
    },
}
...
# Restricted to active training symbols only: MGC, SIL, MES, MNQ, M2K, MYM
# ZN and ZB removed 2026-03 (rate-driven, below signal threshold).
BASELINE_CORRELATIONS: dict[tuple[str, str], float] = {
    ...
    ("MGC", "MYM"): 0.05,  # Gold↔Dow: near-zero (risk switch)
    ...
    ("MNQ", "MES"): 0.92,  # Nasdaq↔S&P: (duplicate for lookup symmetry)
    ("SIL", "MGC"): 0.75,  # Silver↔Gold: (duplicate for lookup symmetry)
    ("SIL", "MES"): 0.35,  # Silver↔S&P: weak-moderate (risk appetite)
    ("M2K", "MNQ"): 0.80,  # Russell↔Nasdaq: (duplicate for lookup symmetry)
    ("MYM", "MNQ"): 0.95,  # Dow↔Nasdaq: very strong index correlation
}
