...
class TestGetDispatcher:
    ...
    def test_dispatcher_reads_discord_env(self, monkeypatch):
        reset_dispatcher()
        monkeypatch.setenv("DISCORD_WEBHOOK_GENERAL", "https://discord.com/api/webhooks/env_test")
        d = get_dispatcher()
        assert "Discord" in d.channels_configured
    ...
    def test_dispatcher_reads_discord_env_only(self, monkeypatch):
        ...
        reset_dispatcher()
        monkeypatch.setenv("DISCORD_WEBHOOK_GENERAL", "https://discord.com/api/webhooks/env_only")
        d = get_dispatcher()
...
# ═══════════════════════════════════════════════════════════════════════════
# Three-channel Discord routing
# ═══════════════════════════════════════════════════════════════════════════


class TestThreeChannelDiscord:
    """Tests for the three-channel (general/signals/analysis) Discord routing."""

    def test_webhook_general_only_has_channels(self):
        """Dispatcher with only webhook_general should report has_channels True."""
        d = _fresh_dispatcher(webhook_general="https://discord.com/api/webhooks/general")
        assert d.has_channels is True
        assert "Discord" in d.channels_configured

    def test_webhook_signals_only_has_channels(self):
        d = _fresh_dispatcher(webhook_signals="https://discord.com/api/webhooks/signals")
        assert d.has_channels is True
        assert "Discord" in d.channels_configured

    def test_webhook_analysis_only_has_channels(self):
        d = _fresh_dispatcher(webhook_analysis="https://discord.com/api/webhooks/analysis")
        assert d.has_channels is True
        assert "Discord" in d.channels_configured

    def test_all_three_webhooks_single_discord_entry(self):
        """With all 3 webhooks, channels_configured should still return ['Discord']."""
        d = _fresh_dispatcher(
            webhook_general="https://discord.com/api/webhooks/general",
            webhook_signals="https://discord.com/api/webhooks/signals",
            webhook_analysis="https://discord.com/api/webhooks/analysis",
        )
        assert d.channels_configured == ["Discord"]

    def test_no_webhooks_no_channels(self):
        d = _fresh_dispatcher()
        assert d.has_channels is False
        assert d.channels_configured == []

    def test_discord_webhook_compat_alias(self):
        """The legacy discord_webhook param should map to webhook_general."""
        d = _fresh_dispatcher(discord_webhook="https://discord.com/api/webhooks/legacy")
        assert d.webhook_general == "https://discord.com/api/webhooks/legacy"
        assert d.has_channels is True

    def test_webhook_general_takes_priority_over_discord_webhook_alias(self):
        """webhook_general should win over discord_webhook alias when both provided."""
        d = _fresh_dispatcher(
            webhook_general="https://discord.com/api/webhooks/explicit",
            discord_webhook="https://discord.com/api/webhooks/alias",
        )
        assert d.webhook_general == "https://discord.com/api/webhooks/explicit"

    def test_get_webhook_signals_falls_back_to_general(self):
        """If webhook_signals is empty, _get_webhook('signals') returns webhook_general."""
        d = _fresh_dispatcher(webhook_general="https://discord.com/api/webhooks/general")
        assert d._get_webhook("signals") == "https://discord.com/api/webhooks/general"

    def test_get_webhook_analysis_falls_back_to_general(self):
        """If webhook_analysis is empty, _get_webhook('analysis') returns webhook_general."""
        d = _fresh_dispatcher(webhook_general="https://discord.com/api/webhooks/general")
        assert d._get_webhook("analysis") == "https://discord.com/api/webhooks/general"

    def test_get_webhook_signals_returns_signals_url(self):
        d = _fresh_dispatcher(
            webhook_general="https://discord.com/api/webhooks/general",
            webhook_signals="https://discord.com/api/webhooks/signals",
        )
        assert d._get_webhook("signals") == "https://discord.com/api/webhooks/signals"

    def test_get_webhook_analysis_returns_analysis_url(self):
        d = _fresh_dispatcher(
            webhook_general="https://discord.com/api/webhooks/general",
            webhook_analysis="https://discord.com/api/webhooks/analysis",
        )
        assert d._get_webhook("analysis") == "https://discord.com/api/webhooks/analysis"

    def test_get_webhook_general_returns_general_url(self):
        d = _fresh_dispatcher(webhook_general="https://discord.com/api/webhooks/general")
        assert d._get_webhook("general") == "https://discord.com/api/webhooks/general"

    def test_send_signal_no_channels_returns_false_with_empty_webhooks(self):
        d = _fresh_dispatcher(webhook_general="", webhook_signals="", webhook_analysis="")
        result = d.send_signal(signal_key="s1", title="T", message="M")
        assert result is False

    def test_regime_change_uses_analysis_channel(self):
        """send_regime_change should resolve the analysis channel internally."""
        d = _fresh_dispatcher(
            webhook_analysis="https://discord.com/api/webhooks/analysis",
        )
        # No channels would be resolved if analysis channel missing; with it set, has_channels True
        assert d.has_channels is True

    def test_confluence_alert_uses_analysis_channel(self):
        """send_confluence_alert (score=3) should resolve analysis channel."""
        d = _fresh_dispatcher(
            webhook_analysis="https://discord.com/api/webhooks/analysis",
        )
        assert d.has_channels is True

    def test_risk_alert_uses_general_channel(self):
        """send_risk_alert should resolve the general channel."""
        d = _fresh_dispatcher(
            webhook_general="https://discord.com/api/webhooks/general",
        )
        assert d.has_channels is True

    def test_discord_bot_token_stored(self):
        """discord_bot_token should be stored on the dispatcher."""
        d = _fresh_dispatcher(discord_bot_token="Bot abc123")
        assert d.discord_bot_token == "Bot abc123"

    def test_discord_bot_token_empty_by_default(self):
        d = _fresh_dispatcher()
        assert d.discord_bot_token == ""

    def test_get_dispatcher_reads_webhook_general_env(self, monkeypatch):
        reset_dispatcher()
        monkeypatch.setenv("DISCORD_WEBHOOK_GENERAL", "https://discord.com/api/webhooks/gen")
        monkeypatch.delenv("DISCORD_WEBHOOK_GENERAL", raising=False)
        d = get_dispatcher()
        assert d.webhook_general == "https://discord.com/api/webhooks/gen"
        assert "Discord" in d.channels_configured
        reset_dispatcher()

    def test_get_dispatcher_reads_webhook_signals_env(self, monkeypatch):
        reset_dispatcher()
        monkeypatch.setenv("DISCORD_WEBHOOK_SIGNALS", "https://discord.com/api/webhooks/sig")
        d = get_dispatcher()
        assert d.webhook_signals == "https://discord.com/api/webhooks/sig"
        assert "Discord" in d.channels_configured
        reset_dispatcher()

    def test_get_dispatcher_reads_webhook_analysis_env(self, monkeypatch):
        reset_dispatcher()
        monkeypatch.setenv("DISCORD_WEBHOOK_ANALYSIS", "https://discord.com/api/webhooks/ana")
        d = get_dispatcher()
        assert d.webhook_analysis == "https://discord.com/api/webhooks/ana"
        assert "Discord" in d.channels_configured
        reset_dispatcher()

    def test_get_dispatcher_reads_bot_token_env(self, monkeypatch):
        reset_dispatcher()
        monkeypatch.setenv("DISCORD_BOT_TOKEN", "Bot test_token_123")
        d = get_dispatcher()
        assert d.discord_bot_token == "Bot test_token_123"
        reset_dispatcher()
        assert "Discord" in d.channels_configured
        reset_dispatcher()
