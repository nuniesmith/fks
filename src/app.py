...
from cache import price_cache, session_manager
...
from framework.config.constants import ALTS, DISCORD_WEBHOOK_URL, MAINS, SYMBOLS, TIMEFRAMES
...
with st.sidebar:
    ...
    if accounts:
        account_options = {
            f"{acc.name} (${float(acc.current_balance):,.2f})": acc.id
            for acc in accounts
        }
        ...
        selected_account = next(
            acc for acc in accounts if acc.id == selected_account_id
        )
        col1, col2 = st.columns(2)
        with col1:
            st.metric("Balance", f"${float(selected_account.current_balance):,.2f}")
        with col2:
            pnl = float(selected_account.current_balance) - float(
                selected_account.initial_balance
            )
            st.metric(
                "PnL",
                f"${pnl:,.2f}",
                f"{(pnl/float(selected_account.initial_balance)*100):.2f}%",
            )
    else:
        ...
        with st.expander("➕ Create Account"):
            ...
            initial_bal = st.number_input(
                "Initial Balance", min_value=100.0, value=10000.0
            )
...
with tab1:
    ...
    auto_refresh = st.checkbox(
        "Auto-refresh (every 5 seconds)", value=False, key="auto_refresh_prices"
    )
    ...
    chart_timeframe = st.selectbox(
        "Timeframe", TIMEFRAMES, index=5, key="live_chart_tf"
    )
        ...
        with cols[idx % 2]:
            try:
                ...
                if not df.empty:
                    ...
                    df["close"].plot(
                        ax=ax, title=f"{symbol} - {chart_timeframe}", color="blue"
                    )
...
with tab2:
        ...
        with st.spinner("Loading data..."):
            try:
                ...
                if df_prices:
                    ...
                    st.success(
                        f"✅ Loaded {len(common_index)} candles from {common_index[0].date()} to {common_index[-1].date()}"
                    )
                    save_session_state()
                else:
    ...
    if "df_prices" in st.session_state and st.session_state.df_prices:
        ...
        st.session_state.df_prices[selected_symbol]["close"].plot(
            ax=ax, title=f"{selected_symbol} Price History"
        )
        ax.grid(True, alpha=0.3)
        st.pyplot(fig)
        plt.close()
    ...
    if "df_prices" in st.session_state:
        ...
        with col1:
            n_trials = st.number_input(
                "Optimization Trials", min_value=10, max_value=500, value=50
            )
            ...
            if st.button("▶️ Run Backtest"):
                with st.spinner("Running backtest..."):
                    metrics, returns, cum_ret, trades = run_backtest(
                        st.session_state.df_prices, **st.session_state.best_params
                    )
    ...
    if "selected_account_id" in st.session_state:
        account = next(
            (
                ...
                if acc.id == st.session_state.selected_account_id
            ),
            None,
        )
        if account:
                ...
                if st.button("🎯 Generate Signal"):
                    with st.spinner("Analyzing market..."):
                        try:
                            ...
                            if signal == 1:
                                st.success(
                                    "📈 Signal: HOLD ASSETS (50% BTC/ETH mix, 50% Alts)"
                                )
                            else:
                            ...
                            if suggestions:
                                st.dataframe(
                                    pd.DataFrame(suggestions), use_container_width=True
                                )
                        except Exception as e:
                            st.error(f"Error: {e}")
            else:
...
with tab7:
    ...
    use_env = st.checkbox(
        "Use .env DISCORD_WEBHOOK_URL", value=bool(DISCORD_WEBHOOK_URL)
    )
...
with tab8:
    ...
    st.caption(
        "Ask questions about your trading history, strategies, and get AI-powered insights"
    )
    ...
    if rag_available:
                ...
                with st.spinner("🤔 Thinking..."):
                    try:
                        ...
                        if result.get("sources"):
                            with st.expander(
                                f"📚 View {len(result['sources'])} Sources"
                            ):
                                for i, source in enumerate(result["sources"], 1):
                                    st.markdown(
                                        f"**Source {i}** (Relevance: {source.get('similarity_score', 0):.1%})"
                                    )
                                    ...
                                    if metadata:
                                        st.caption(
                                            f"Symbol: {metadata.get('symbol', 'N/A')} | Type: {metadata.get('doc_type', 'N/A')}"
                                        )
                                    st.markdown("---")
                ...
                with st.spinner("Analyzing..."):
                    try:
                        result = rag_service.suggest_strategy(
                            symbol=strat_symbol, market_condition=market_cond
                        )
                        st.success("Strategy Recommendation:")
                        st.markdown(result["strategy"])
                        st.caption(
                            f"Based on {result['sources_count']} historical documents"
                        )
                    except Exception as e:
                        st.error(f"Error: {e}")
        ...
        if "query_history" in st.session_state and st.session_state.query_history:
            ...
            with st.expander(
                ...
                for i, item in enumerate(
                    reversed(st.session_state.query_history[-10:]), 1
                ):
        ...
        with col3:
            ...
            st.text(
                f"Model: {rag_service.local_model if rag_service.use_local else rag_service.openai_model}"
            )
...
st.sidebar.caption(
    f"Last updated: {datetime.now(TIMEZONE).strftime('%Y-%m-%d %H:%M:%S')}"
)
