...
def sample_prices(
    yf, symbol: str, period: str, interval: str, max_rows: int = 200
) -> dict[str, Any]:
    try:
...
def daily_ohlcv(
    yf, symbol: str, start: str | None, end: str | None, period: str
) -> dict[str, Any]:
    try:
        if start or end:
            df = yf.download(
...
def fks_ohlcv(
    yf, symbol: str, interval: str, period: str, start: str | None, end: str | None
) -> dict[str, Any]:
    tf_map = {
        "1m": "1m",
        "5m": "5m",
    ...
    try:
        if start or end:
            df = yf.download(
                symbol, start=start, end=end, interval=yf_interval, progress=False
            )
        else:
            df = yf.download(
                symbol, period=period, interval=yf_interval, progress=False
            )
    except Exception:
        df = None
    if df is None or getattr(df, "empty", True):
