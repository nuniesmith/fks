...
Usage:
    from trading.strategies.daily.daily_plan import generate_daily_plan, DailyPlan
...
from trading.strategies.daily.bias_analyzer import (
    BiasDirection,
    DailyBias,
...
def _compute_signal_quality_for_asset(asset_name: str) -> float:
    ...
    try:
        ...
        from core.cache import get_data
...
def _fetch_asset_data(asset_name: str) -> tuple[float, float]:
    ...
    try:
        from analysis.volatility import kmeans_volatility_clusters
        from services.data.engine_data_client import get_client
...
def _fetch_grok_context(
    ...
    try:
        from integrations.grok_helper import _call_grok, run_morning_briefing
    except ImportError:
...
def generate_daily_plan(
    ...
    if include_grok and os.getenv("XAI_API_KEY", "").strip():
        ...
        try:
            from integrations.grok_helper import (
