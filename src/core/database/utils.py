...
def get_latest_ohlcv_time(symbol: str, timeframe: str) -> Optional[datetime]:
    """Get the timestamp of the most recent OHLCV data"""
    session = Session()
    try:
        result = session.query(func.max(OHLCVData.time)).filter(
            and_(
                OHLCVData.symbol == symbol,
                OHLCVData.timeframe == timeframe
            )
...
def get_oldest_ohlcv_time(symbol: str, timeframe: str) -> Optional[datetime]:
    """Get the timestamp of the oldest OHLCV data"""
    session = Session()
    try:
        result = session.query(func.min(OHLCVData.time)).filter(
            and_(
                OHLCVData.symbol == symbol,
                OHLCVData.timeframe == timeframe
            )
...
def update_sync_status(
    ...
    try:
        sync = session.query(SyncStatus).filter(
            and_(
                SyncStatus.symbol == symbol,
                SyncStatus.timeframe == timeframe
            )
        ).first()
...
def get_sync_status(symbol: Optional[str] = None, timeframe: Optional[str] = None) -> List[Dict]:
...
def get_balance_history(
    ...
    try:
        ...
        df = pd.DataFrame([{
            ...
            'cumulative_pnl': float(r.cumulative_pnl) if r.cumulative_pnl else 0
        } for r in results])
...
def get_active_strategy_parameters(strategy_name: str) -> Optional[StrategyParameters]:
    """Get active strategy parameters"""
    session = Session()
    try:
        return session.query(StrategyParameters).filter(
            and_(
