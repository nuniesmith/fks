...
class MassiveDataProvider:
    ...
    def resolve_front_month_at_date(
        self,
        product_code: str,
        date_str: str,
        end_date_str: str | None = None,
    ) -> str | None:
        """Resolve a product code to the front-month contract active on a specific date.

        This is the historical analogue of ``resolve_front_month`` — instead of
        finding today's active contract it queries the Massive contracts endpoint
        with ``date=date_str`` so that backfill chunks use the correct contract
        ticker for the period being fetched (e.g. M2KH5 for Jan 2025 data rather
        than the current front-month M2KH6).

        When ``end_date_str`` is supplied the resolver prefers a contract whose
        last_trade_date falls **on or after** that date.  This matters for
        products with dense expiry calendars (e.g. Micro Silver / SIL which has
        monthly contracts): without the end-date hint the nearest-expiry contract
        is chosen, which may expire mid-chunk and return almost no bars for the
        latter half of the window.

        Uses a per-date TTL cache keyed on ``"{product_code}@{date_str}@{end_date_str}"``
        with the same 1-hour TTL as the current-contract cache.

        Args:
            product_code:  Massive product code, e.g. ``"M2K"``, ``"SIL"``.
            date_str:      ISO date string ``"YYYY-MM-DD"`` — start of the window.
            end_date_str:  Optional ISO date string ``"YYYY-MM-DD"`` — end of the
                           window.  When set, contracts expiring before this date
                           are skipped so the chosen contract covers the full chunk.

        Returns:
            The active contract ticker string (e.g. ``"M2KH5"``) or ``None`` on
            failure.  Falls back to the current front-month if the date-specific
            query yields nothing.
        """
        if not self.is_available or self._client is None:
            return None

        cache_key = f"{product_code}@{date_str}@{end_date_str or ''}"
        now = time.time()
        ...
        def _covers_end(c) -> bool:
            """True if the contract expires on or after end_date_str."""
            if not end_date_str:
                return True
            ltd = str(getattr(c, "last_trade_date", None) or "")
            return bool(ltd) and ltd >= end_date_str

        # --- Tier 1: active contracts on the requested date ---
        try:
            with self._lock:
                contracts = list(
                    client.list_futures_contracts(
            ...
            active = [c for c in contracts if _is_outright(c)]
            if active:
                active.sort(key=_sort_by_expiry)
                # Prefer the nearest contract whose expiry covers end_date_str.
                # Fall back to the absolute nearest if none cover the full window
                # (e.g. the chunk spans a roll that has no further contracts listed).
                covering = [c for c in active if _covers_end(c)]
                chosen = covering[0] if covering else active[0]
                ticker = str(getattr(chosen, "ticker", ""))
                if ticker:
                    with self._lock:
                        self._contract_cache[cache_key] = (ticker, now)
                    logger.debug(
                        "Resolved %s on %s (end=%s) → %s (expires: %s)",
                        product_code,
                        date_str,
                        end_date_str or "—",
                        ticker,
                        getattr(chosen, "last_trade_date", "?"),
                    )
                    return ticker
        except Exception as exc:
            logger.debug("Date-based contract resolution failed for %s on %s: %s", product_code, date_str, exc)

        # --- Tier 2: derive contract code from CME quarterly cycle ---
        # When the contracts endpoint errors or returns nothing for a historical
        # date (common for dates near Massive's data-availability boundary),
        # construct the ticker directly from the standard CME expiry schedule:
        #   H = March  (exp ~3rd Friday of Mar)
        #   M = June   (exp ~3rd Friday of Jun)
        #   U = September (exp ~3rd Friday of Sep)
        #   Z = December  (exp ~3rd Friday of Dec)
        #
        # The front-month on any given date is the nearest quarterly expiry
        # that has not yet passed (we use the 3rd Friday of the expiry month
        # as a conservative rollover boundary — most traders roll a week
        # before, but fetching the old contract up to actual expiry is safe
        # because Massive still has that data).
        try:
            target = datetime.strptime(date_str, "%Y-%m-%d")
            year = target.year

            # Build list of quarterly expiry months and their month-codes
            _cycle = [(3, "H"), (6, "M"), (9, "U"), (12, "Z")]

            def _third_friday(y: int, m: int) -> datetime:
                """Return the 3rd Friday of month m in year y."""
                import calendar

                # Find first Friday
                first_day, days_in_month = calendar.monthrange(y, m)
                # first_day: 0=Mon … 4=Fri
                first_friday = (4 - first_day) % 7 + 1
                return datetime(y, m, first_friday + 14)

            # Collect all quarterly expiries for this year and next
            candidates: list[tuple[datetime, str]] = []
            for y in (year - 1, year, year + 1):
                for exp_month, code in _cycle:
                    exp_dt = _third_friday(y, exp_month)
                    candidates.append((exp_dt, f"{product_code}{code}{str(y)[-1]}"))

            # Pick the nearest expiry that is on or after the target date
            candidates.sort(key=lambda x: x[0])
            for exp_dt, derived_ticker in candidates:
                if exp_dt >= target:
                    with self._lock:
                        self._contract_cache[cache_key] = (derived_ticker, now)
                    logger.debug(
                        "Resolved %s on %s → %s (tier 2: CME cycle derivation, expires ~%s)",
                        product_code,
                        date_str,
                        derived_ticker,
                        exp_dt.strftime("%Y-%m-%d"),
                    )
                    return derived_ticker
        except Exception as exc:
            logger.debug("CME cycle derivation failed for %s on %s: %s", product_code, date_str, exc)

        # --- Tier 3: fall back to current front-month ---
        # Last resort only — this will be wrong for dates > 1 quarter ago
        # but is better than returning None and skipping the chunk entirely.
        logger.debug(
            "No historical contract found for %s on %s — falling back to current front-month",
            product_code,
            date_str,
        )
        return self.resolve_front_month(product_code)

    def resolve_from_yahoo_at_date(
        self,
        yahoo_ticker: str,
        date_str: str,
        end_date_str: str | None = None,
    ) -> str | None:
        """Resolve a Yahoo-style ticker to the Massive contract active on a specific date.

        Combines ``YAHOO_TO_MASSIVE_PRODUCT`` mapping with ``resolve_front_month_at_date``
        so callers don't need to know the product code.

        Args:
            yahoo_ticker:  Yahoo Finance-style ticker, e.g. ``"M2K=F"``.
            date_str:      ISO date string ``"YYYY-MM-DD"`` — start of the window.
            end_date_str:  Optional ISO date string ``"YYYY-MM-DD"`` — end of the
                           window, passed through to ``resolve_front_month_at_date``
                           so that dense-expiry products (e.g. SIL) pick a contract
                           covering the full chunk rather than the nearest expiry.

        Returns:
            Active contract ticker string or ``None`` if the product code is unknown
            or resolution fails entirely.
        """
        ...
        if product_code is None:
            ...
            return None
        return self.resolve_front_month_at_date(product_code, date_str, end_date_str=end_date_str)
