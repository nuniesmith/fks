...
- **`export`** reads `state.manifest`, copies the listed files, dumps the listed
  ...
  the result to `fks-state/snapshots/fks-state-<UTC>.tar.gz.age`. The private repo
  is just encrypted transport — if it leaked, the contents stay sealed to your key.
- **`import`** pulls the latest snapshot, decrypts it with your key, and restores
  the files into place + replays the DB dumps via `psql`. It prompts before
  overwriting; `--dry-run` previews without touching anything.
...
| Strategy configs (the edge) | the bot repos' `funding.toml`, `spot-portfolio.toml` (add their paths) |
| Live state | journals, open-position JSON |
