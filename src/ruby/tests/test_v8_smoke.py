...
# Valid label values accepted by BreakoutDataset (RETRAIN-I1: 11-label scheme)
LABELS = [
    "excellent_long",
    "excellent_short",  # hit TP2/TP3 → class 2
    "good_long",
    "good_short",  # hit TP1 only → class 2
    "marginal_long",
    "marginal_short",  # timed out in profit → class 1
    "bad_long",
    "bad_short",  # SL hit / timeout at loss → class 0
    "choppy_long",
    "choppy_short",  # SL hit ≤15 bars → class 0
    "no_setup",  # no valid breakout → class 0
]
...
class TestV8Architecture:
    ...
    def test_forward_with_embeddings(self):
        """Full forward pass with asset embedding IDs produces (B, 3) logits."""
        from lib.analysis.ml.breakout_cnn import HybridBreakoutCNN
        ...
        assert out.shape == (B, 3), f"Expected (B,3) logits, got {out.shape}"
...
class TestBreakoutDataset:
    ...
    def test_getitem_shapes(self, synthetic_dataset):
        ...
        assert int(label) in (0, 1, 2), f"Label must be 0, 1, or 2, got {label}"
    ...
    def test_label_balance(self, synthetic_dataset):
        """Synthetic dataset should have all three target classes represented."""
        from lib.analysis.ml.breakout_cnn import _TORCH_AVAILABLE, BreakoutDataset
        ...
        # 3-class scheme: 0=bad/choppy/no_setup, 1=marginal, 2=good/excellent
        assert 0 in labels, "Must have at least one class-0 label (bad/choppy/no_setup)"
        assert 1 in labels, "Must have at least one class-1 label (marginal)"
        assert 2 in labels, "Must have at least one class-2 label (good/excellent)"
...
class TestV8GradAccumAndMixup:
    ...
    def test_grad_accumulation_accumulates_gradients(self):
        ...
        class_ids = torch.randint(0, NUM_ASSET_CLASSES, (B,))
        asset_ids = torch.randint(0, NUM_ASSETS, (B,))
        labels = torch.randint(0, 3, (B,))  # 3-class: 0=bad, 1=marginal, 2=good/excellent
    ...
    def test_label_smoothing_loss(self):
        ...
        B = 8
        logits = torch.randn(B, 3)  # 3-class output (RETRAIN-I1)
