...
class PineScriptGenerator:
    ...
    def __init__(self, base_dir: str, logger_level: int = logging.INFO):
        ...
        # Lint results keyed by indicator_type — populated on each (re)generation
        self._lint_results: dict[str, dict] = {}
    ...
    def lint_script(self, script: str) -> dict:
        """Lint a Pine Script string and return a structured result.

        Runs two checks:
        - **W001** — missing or outdated ``//@version=`` annotation.
        - **S001** — AST syntax error detected by the ``pynescript`` parser.
          Falls back gracefully (``S000`` warning) when the package is absent.

        Returns:
            dict with keys:
                ``ok`` (bool)      — ``True`` when no errors were found.
                ``errors`` (list)  — Fatal issues that will likely prevent the
                                     script from compiling on TradingView.
                ``warnings`` (list)— Non-fatal style / best-practice issues.
        """
        errors: list[str] = []
        warnings: list[str] = []

        # ── W001: version annotation ─────────────────────────────────────────
        version_match = re.search(r"//@version=(\d+)", script)
        if version_match is None:
            warnings.append("W001: Missing //@version= annotation")
        else:
            version = int(version_match.group(1))
            if version < 5:
                warnings.append(f"W001: Using deprecated Pine Script v{version} — consider upgrading to v6")

        # ── S001: AST syntax parse ────────────────────────────────────────────
        try:
            import pynescript  # type: ignore[import-untyped]

            pynescript.parse(script)  # type: ignore[attr-defined]
        except ImportError:
            warnings.append("S000: pynescript not installed — AST syntax check skipped")
        except Exception as exc:  # noqa: BLE001
            errors.append(f"S001: Syntax error — {exc}")
    ...
    def generate_full_script(self, indicator_type: str) -> tuple[str, list[str]]:
        ...
        if cached_script:
            # Re-run lint on cache hit so _lint_results is always populated
            # (e.g. after a server restart that cleared the in-memory dict).
            if indicator_type not in self._lint_results:
                self._lint_results[indicator_type] = self.lint_script(cached_script)
        ...
        # Lint the assembled script before caching
        lint_result = self.lint_script(full_script)
        self._lint_results[indicator_type] = lint_result
        if lint_result["errors"]:
            for err in lint_result["errors"]:
                self.logger.error("Pine lint [%s]: %s", indicator_type, err)
        for warn in lint_result["warnings"]:
            self.logger.warning("Pine lint [%s]: %s", indicator_type, warn)
    ...
    def get_indicator_stats(self, indicator_type: str) -> dict[str, Any]:
        ...
        lint = self._lint_results.get(
            indicator_type,
            {"ok": None, "errors": [], "warnings": []},
        )
        return {
            "indicator_type": indicator_type,
