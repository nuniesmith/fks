...
class AssetPricePredictor:
    ...
    def predict_next_movement(
        ...
        if current_data is None or current_data.empty:
            logger.warning(
                "No valid data available for prediction — returning unavailable sentinel "
                "(will NOT use placeholder zeros as fake predictions)."
            )
            return {
                ...
                "direction": None,
                "confidence": 0.0,
                "message": "Prediction unavailable: no market data found for this asset/timeframe.",
            }
    ...
    def _create_placeholder_data(self) -> pd.DataFrame:
        ...
        .. warning::
            **DEAD CODE** — This method is no longer called.  The no-data path
            in :meth:`predict_next_movement` now returns an unavailable sentinel
            (``{"prediction_source": "unavailable", ...}``) directly instead of
            fabricating fake inputs.  The method is retained so that future code
            paths (e.g. backtesting warm-up) can opt in to placeholder data with
            full awareness of the ``attrs["prediction_source"] = "placeholder"``
            tag.

        .. warning::
            Returns fabricated neutral-sentiment data (RSI=50, distances=0, etc.)
            that does NOT reflect actual market conditions.  Any predictions derived
            from this data are meaningless.  The returned DataFrame is tagged with
            ``attrs["prediction_source"] = "placeholder"`` so downstream code can
            detect and discard results built on fake inputs.
        """
        logger.warning(
            "Creating PLACEHOLDER data for %s — predictions derived from this "
            "data are NOT based on real market conditions and should be discarded.",
            self.asset_type,
        )
        now = datetime.now()
        ...
        # Tag the DataFrame so downstream code can detect placeholder origin
        placeholder_data.attrs["prediction_source"] = "placeholder"
    ...
    def _make_predictions(
        ...
        # Propagate placeholder flag so downstream consumers can detect fake predictions
        if latest_data.attrs.get("prediction_source") == "placeholder":
            logger.warning(
                "Predictions for %s are based on PLACEHOLDER data and should not be trusted.",
                self.asset_type,
            )
            results["prediction_source"] = "placeholder"
    ...
    def _make_time_horizon_prediction(
        ...
        # Propagate placeholder flag if present
        if raw_predictions.get("prediction_source") == "placeholder":
            results["prediction_source"] = "placeholder"

        for time_frame, prediction_data in raw_predictions.items():
            # Skip metadata keys that aren't actual prediction entries
