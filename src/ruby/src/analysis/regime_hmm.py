"""Adapted from ruby/lib/analysis/regime.py — HMM-based regime detection.
...
Optional dependency: ``hmmlearn`` (install with ``pip install hmmlearn``).
Also requires ``scikit-learn`` for ``StandardScaler``.

Usage:
    from analysis.regime_hmm import RegimeDetector, detect_regime_hmm
...
def _get_column(df: pd.DataFrame, lowercase: str, capitalized: str) -> pd.Series:
    """Retrieve a DataFrame column, trying lowercase first then capitalized.

    The futures project uses lowercase column names (``close``, ``high``,
    ``low``, ``volume``) while the original Ruby project uses capitalized
    names (``Close``, ``High``, ``Low``, ``Volume``).  This helper
    transparently supports both conventions so the module works regardless
    of the upstream data source.

    Raises ``KeyError`` if neither variant is present.
    """
    ...
    raise KeyError(
        f"DataFrame has neither '{lowercase}' nor '{capitalized}' column. "
...
def _compute_features(df: pd.DataFrame) -> np.ndarray | None:
    ...
    Accepts both lowercase (``close``, ``high``, ``low``, ``volume``) and
    capitalized (``Close``, ``High``, ``Low``, ``Volume``) column names.
    ...
    try:
        close = _get_column(df, "close", "Close").astype(float)
        high = _get_column(df, "high", "High").astype(float)
        low = _get_column(df, "low", "Low").astype(float)
        volume = _get_column(df, "volume", "Volume").astype(float)
    except KeyError as exc:
        logger.warning("Cannot compute features: %s", exc)
        return None
...
def _label_states(model: Any, features: np.ndarray) -> dict[int, str]:
...
class RegimeDetector:
    ...
    def fit(self, df: pd.DataFrame) -> bool:
        ...
        Tries multiple random seeds and selects the model with best
        log-likelihood.  Returns True if fitting succeeded, False otherwise.

        Requires ``hmmlearn`` and ``scikit-learn`` to be installed.
        """
        try:
        ...
        # Try multiple seeds, pick best log-likelihood
        best_model = None
        best_score = -np.inf
    ...
    def detect(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        effective_regime = (
            raw_regime if self._persistence_count >= self.persistence_bars else self._last_regime
        )
