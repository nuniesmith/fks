...
KRAKEN_PAIRS: dict[str, dict[str, str]] = {
    ...
    # ── Meme & alt coins (portfolio targets) ────────────────────────────────
    "Dogecoin": {
        "rest_pair": "XDGEZUSD",
        "ws_pair": "DOGE/USD",
        "internal_ticker": "KRAKEN:DOGEUSD",
        "base": "DOGE",
        "quote": "USD",
    },
    "Optimism": {
    ...
    "Pepe": {
        "rest_pair": "PEPEUSD",
        "ws_pair": "PEPE/USD",
        "internal_ticker": "KRAKEN:PEPEUSD",
        "base": "PEPE",
        "quote": "USD",
    },
    "dogwifhat": {
        "rest_pair": "WIFUSD",
        "ws_pair": "WIF/USD",
        "internal_ticker": "KRAKEN:WIFUSD",
        "base": "WIF",
        "quote": "USD",
    },
    "Fartcoin": {
        "rest_pair": "FARTCOINUSD",
        "ws_pair": "FARTCOIN/USD",
        "internal_ticker": "KRAKEN:FARTCOINUSD",
        "base": "FARTCOIN",
        "quote": "USD",
    },
}
...
class KrakenDataProvider:
    ...
    # ── Kraken Earn (staking / flexible yield) ────────────────────────────

    def get_earn_allocations(self, converted_asset: str = "USD") -> dict[str, Any]:
        """Return current Earn allocations — assets earning yield (requires auth).

        Lists all active earn positions: strategy ID, native asset, amounts
        allocated (bonding / unbonding / total) and rewards earned so far.

        Args:
            converted_asset: Currency to use for converted amounts (default "USD").

        Returns:
            Raw Kraken API response::
                    ...
                    "items": [
                      {
                        ...
                        "native_asset": "USDT",
                        "amount_allocated": {
                          "bonding":   {"native": "0", "converted": "0"},
                          "total":     {"native": "1000.00", "converted": "1000.00"}
                        },
                        "total_rewarded": {"native": "10.00", "converted": "10.00"}
                      }
                    ]
                  }
                }
        """
        return self._private_post("Earn/Allocations", {"converted_asset": converted_asset})
    ...
    def get_earn_strategies(
        ...
        """Return available Earn strategies with APR estimates (requires auth).

        Lists all strategies the account can allocate to.  Filter by ``asset``
        to narrow to a specific coin (e.g. ``"USDT"`` or ``"USDC"``).

        Args:
            asset:  Native asset code to filter by (e.g. ``"USDT"``). ``None`` returns all.
            limit:  Max strategies to return (default 100, API max 1000).
                    ...
                    "items": [
                      {
                        ...
                        "asset": "USDT",
                        "lock_type": {"type": "flex"},
                        "apr_estimate": {"low": "4.00", "high": "6.00"},
                        "user_min_allocation": "1.00",
                        "can_allocate": true,
                        "can_deallocate": true,
        ...
        if asset is not None:
            params["asset"] = asset
        return self._private_post("Earn/Strategies", params)

    def get_staking_assets(self) -> dict[str, Any]:
        """Return assets available for on-chain staking (requires auth).

        This is the older on-chain staking endpoint (as opposed to Earn).
        Returns locked/bonded staking assets and their staking reward info.
        """
        return self._private_post("Staking/Assets", {})

    def get_staking_pending(self) -> dict[str, Any]:
        """Return pending staking transactions (requires auth)."""
        return self._private_post("Staking/Pending", {})
...
# ═══════════════════════════════════════════════════════════════════════════
# Kraken Futures (Derivatives) REST Client
# ═══════════════════════════════════════════════════════════════════════════

# Kraken Futures perpetual symbol map: clean code → API symbol
...
# Reverse map: API symbol → clean code
_FUTURES_SYMBOL_TO_CLEAN: dict[str, str] = {v: k for k, v in FUTURES_SYMBOLS.items()}


def futures_symbol(clean_code: str) -> str:
    """Return the Kraken Trading API symbol for a clean asset code.
    Falls back to clean_code.upper() if not in the map.
    """
    return FUTURES_SYMBOLS.get(clean_code.upper(), clean_code.upper())


class KrakenFuturesClient:
    """REST client for the Kraken Futures (derivatives) API.

    Base URL: https://futures.kraken.com/derivatives/api/v3/
    Docs: https://docs.futures.kraken.com/

    Auth: APIKey header + Authent HMAC-SHA512 signature.
    """
    ...
    def _sign(self, endpoint_path: str, nonce: str, post_data: str = "") -> str:
        ...
        Algorithm (from Kraken Futures official Python SDK):
          1. sha256(post_data + nonce + endpoint_path) → hash_bytes
          2. HMAC-SHA512(hash_bytes, base64.b64decode(api_secret))
          3. base64.b64encode(result)
        """
        sha256_hash = hashlib.sha256((post_data + nonce + endpoint_path).encode("utf-8")).digest()
        sig = hmac.new(base64.b64decode(self._api_secret), sha256_hash, hashlib.sha512)
        return base64.b64encode(sig.digest()).decode()

    def _get(self, path: str) -> dict[str, Any]:
        """Authenticated GET request to the Futures REST API."""
        nonce = str(int(time.time() * 1000))
        auth_sig = self._sign(path, nonce, "")
        session = self._get_session()
        resp = session.get(
            f"{self._BASE_URL}{path}",
            headers={"APIKey": self._api_key, "Authent": auth_sig, "Nonce": nonce},
            timeout=15,
        )
        resp.raise_for_status()
        data = resp.json()
        if data.get("result") not in ("success", None) and "error" in data:
            raise ValueError(f"Kraken Trading API error: {data.get('error')}")
        return data

    def _post(self, path: str, data: dict | None = None) -> dict:
        """Authenticated POST request to the Futures REST API.

        Uses x-www-form-urlencoded body, Kraken Futures HMAC-SHA512 auth.
        """
        post_body = urllib.parse.urlencode(data or {})
        nonce = str(int(time.time() * 1000))
        auth_sig = self._sign(path, nonce, post_body)
        session = self._get_session()
        resp = session.post(
            f"{self._BASE_URL}{path}",
            data=post_body,
        ...
        resp.raise_for_status()
        result = resp.json()
        if result.get("result") not in ("success", None) and "error" in result:
            raise ValueError(f"Kraken Trading API error: {result.get('error')}")
        return result
    ...
    def place_order(
        ...
        Args:
            symbol:          Futures symbol e.g. 'PF_XBTUSD' (or clean code 'BTC').
            side:            'buy' or 'sell'.
            size:            Order size in contracts.
            order_type:      'mkt' (market) or 'lmt' (limit). Default 'mkt'.
            limit_price:     Required when order_type='lmt'.
            client_order_id: Optional client-assigned order ID for deduplication.
            reduce_only:     If True, only reduces an existing position.

        Returns:
            Raw Kraken Trading API response with 'sendStatus' key.
        """
        sym = futures_symbol(symbol) if not symbol.startswith("PF_") else symbol
        data: dict = {
    ...
    def cancel_order(self, order_id: str) -> dict:
        ...
        Args:
            order_id: Kraken Futures order ID or client order ID.

        Returns:
            Raw API response with 'cancelStatus' key.
        """
    ...
    def edit_order(
        ...
        Args:
            ...
            size:        New size (contracts). None = keep current.
            limit_price: New limit price. None = keep current.

        Returns:
            Raw API response with 'editStatus' key.
        """
        data: dict = {"orderId": order_id}
        if size is not None:
            data["size"] = str(size)
        if limit_price is not None:
            data["limitPrice"] = str(limit_price)
        return self._post("/editorder", data)

    def get_tickers(self) -> dict:
        """Return all available futures tickers (public endpoint).

        Returns dict keyed by symbol with bid, ask, last, vol, etc.
        """
        session = self._get_session()
        resp = session.get(f"{self._BASE_URL}/tickers", timeout=15)
        resp.raise_for_status()
        data = resp.json()
        # Response: {"result": "success", "tickers": [...]}
        tickers_list = data.get("tickers", [])
        return {t["symbol"]: t for t in tickers_list if "symbol" in t}

    def get_ticker(self, symbol: str) -> dict:
        """Return ticker for a single futures symbol (public).

        Args:
            symbol: e.g. 'PF_XBTUSD' or clean code 'BTC'.
        """
        sym = futures_symbol(symbol) if not symbol.startswith("PF_") else symbol
    ...
    def get_orderbook(self, symbol: str, depth: int = 10) -> dict:
        """Return order book for a symbol (public).

        Args:
            symbol: Futures symbol (e.g. 'PF_XBTUSD') or clean code.
    ...
    def get_fills(self, last_fill_time: str | None = None) -> dict[str, Any]:
        """Return recent fills (trade history).

        Args:
            last_fill_time: ISO8601 datetime — return fills after this time only.
        """
        path = "/fills"
        if last_fill_time:
            path += f"?lastFillTime={last_fill_time}"
        return self._get(path)

    def health(self) -> dict[str, Any]:
        """Return connectivity/auth health check."""
        result: dict[str, Any] = {
            "available": True,
            "authenticated": self.has_auth,
            "connected": False,
        ...
        try:
            ...
            result["connected"] = True
            result["account_count"] = len(data.get("accounts", {}))
        except Exception as exc:
            result["error"] = str(exc)
        return result


def get_kraken_futures_client() -> KrakenFuturesClient:
    """Return an authenticated KrakenFuturesClient using env-var credentials."""
...
class KrakenFuturesWebSocket:
    """Async WebSocket client for Kraken Futures real-time data.

    Connects to wss://futures.kraken.com/ws/v2 and subscribes to:
      - ``ticker`` — real-time price/volume updates per symbol (public)
      - ``trade``  — individual trade prints (public)
      - ``fills``  — your own fills (requires auth challenge)
      - ``open_positions`` — real-time position updates (requires auth)

    Auth uses the Kraken Futures challenge-response scheme:
      1. Subscribe to ``challenge`` feed with APIKey.
      2. Receive a signed_challenge string.
      3. Sign it: HMAC-SHA512(sha256(challenge), base64.b64decode(api_secret)).
      4. Send ``subscribe`` with ``APIKey`` + ``signed_challenge``.

    Usage::

        ws = KrakenFuturesWebSocket(api_key, api_secret)
        ws.on_ticker = lambda sym, data: print(sym, data["last"])
        ws.on_fill   = lambda fill: print("fill", fill)
        await ws.start(["PF_XBTUSD", "PF_ETHUSD"])
        # ...
        await ws.stop()

    Thread safety: callbacks are invoked from the asyncio event loop.
    """

    _WS_URL = "wss://futures.kraken.com/ws/v2"
    ...
    def __init__(self, api_key: str = "", api_secret: str = "") -> None:
        ...
        # Callbacks — override these before calling start()
        self.on_ticker: Any = None  # (symbol: str, data: dict) -> None
        self.on_trade: Any = None  # (symbol: str, trade: dict) -> None
        self.on_fill: Any = None  # (fill: dict) -> None
        self.on_position: Any = None  # (position: dict) -> None
        self.on_error: Any = None  # (error: str) -> None

        logger.info(
            "KrakenFuturesWebSocket created (auth=%s)",
            "yes" if self._api_key else "no",
        )
    ...
    def _sign_challenge(self, challenge: str) -> str:
        """Sign a challenge string using Kraken Futures auth scheme."""
        sha256_hash = hashlib.sha256(challenge.encode("utf-8")).digest()
        sig = hmac.new(base64.b64decode(self._api_secret), sha256_hash, hashlib.sha512)
        return base64.b64encode(sig.digest()).decode()

    async def start(self, symbols: list[str]) -> None:
        """Connect and subscribe to feeds for *symbols*.

        Args:
            symbols: List of Kraken Futures symbols (e.g. ['PF_XBTUSD']).
        """
        if self._running:
            return
        self._subscribed_symbols = symbols
        self._running = True
        self._task = asyncio.create_task(self._run_loop(), name="kraken-futures-ws")
        logger.info("KrakenFuturesWebSocket starting for symbols: %s", symbols)

    async def stop(self) -> None:
        """Disconnect and stop the WebSocket loop."""
        self._running = False
        if self._ws:
            try:
        ...
        if self._task and not self._task.done():
            ...
            with contextlib.suppress(asyncio.CancelledError):
                await self._task
        logger.info("KrakenFuturesWebSocket stopped")
    ...
    async def _run_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                logger.warning(
                    "KrakenFuturesWebSocket error (reconnecting in %.1fs): %s",
                    backoff,
                    exc,
                )
                if self.on_error:
    ...
    async def _connect_and_run(self) -> None:
        """Open the WebSocket, subscribe, and process messages until disconnected."""
        try:
        ...
        async with websockets.connect(
            ...
            self._ws = ws
            logger.info("KrakenFuturesWebSocket connected to %s", self._WS_URL)
            ...
            async for message in ws:
                ...
                try:
                    ...
                    logger.debug("KrakenFuturesWebSocket message error: %s", exc)
    ...
    async def _authenticate(self, ws) -> None:
        ...
        # Wait for challenge response (with timeout)
        for _ in range(10):
            try:
                msg = await asyncio.wait_for(ws.recv(), timeout=5.0)
                data = json.loads(msg)
                if data.get("event") == "challenge" and "message" in data:
                    challenge = data["message"]
                    signed = self._sign_challenge(challenge)
                    # Subscribe to authenticated feeds
                    await ws.send(
                        json.dumps(
                            {
                                "event": "subscribe",
                    ...
                    logger.info("KrakenFuturesWebSocket authenticated")
                    return
            except TimeoutError:
                continue
        logger.warning("KrakenFuturesWebSocket: auth challenge timed out")

    async def _subscribe_public(self, ws) -> None:
        """Subscribe to public ticker and trade feeds for all symbols."""
                ...
                json.dumps(
                    {
                        "event": "subscribe",
                        "feed": "ticker",
                        "product_ids": self._subscribed_symbols,
                    }
                )
            )
            await ws.send(
        ...
        logger.debug(
            "KrakenFuturesWebSocket: subscribed to public feeds for %s",
    ...
    def _handle_message(self, data: dict) -> None:
        ...
        if event in ("subscribed", "subscribe", "challenge", "info", "heartbeat"):
            return  # control messages, ignore

        if feed == "ticker" and data.get("product_id") or feed == "ticker_lite" and data.get("product_id"):
            if self.on_ticker:
                self.on_ticker(data["product_id"], data)

        elif feed == "trade" and data.get("product_id"):
            if self.on_trade:
                self.on_trade(data["product_id"], data)
            ...
            for fill in fills:
                if self.on_fill:
                    self.on_fill(fill)
            # Also fire for the snapshot case (single fill dict)
            if "order_id" in data and self.on_fill:
                self.on_fill(data)
            ...
            logger.warning("KrakenFuturesWebSocket alert: %s", data.get("message"))
