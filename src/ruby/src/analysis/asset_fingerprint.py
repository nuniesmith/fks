...
Public API::
    ...
    from analysis.asset_fingerprint import (
...
logger = get_logger("analysis.asset_fingerprint")
...
class AssetFingerprint:
    ...
    def to_plan_cards(self) -> list[dict[str, str]]:
        """Convert fingerprint to ``[{t, v}, ...]`` cards for the trading plan UI.

        Each card has a short title (``t``) and a human-readable description
        (``v``) so the frontend can call ``.map()`` directly on the list.
        """
        cards: list[dict[str, str]] = []

        # ── Volatility ────────────────────────────────────────────────────
        atr = self.typical_daily_range_atr
        if atr >= 1.5:
        ...
        cards.append(
            {
                "t": "Volatility",
                "v": f"{vol_label} asset — ATR multiplier {atr:.2f}x typical daily range.",
            }
        )

        # ── Session concentration ─────────────────────────────────────────
        sc = self.session_concentration
        dom = sc.dominant_session.capitalize()
        cards.append(
            {
                "t": "Session Behavior",
                "v": (
                    f"Most range generated in the {dom} session "
                    f"(overnight {sc.overnight_pct:.0%} · London {sc.london_pct:.0%} "
                    f"· US {sc.us_pct:.0%} · settle {sc.settle_pct:.0%})."
                ),
            }
        )

        # ── Breakout follow-through ───────────────────────────────────────
        ft = self.breakout_follow_through
        if ft.sample_count > 0:
            rate_pct = ft.follow_through_rate * 100
            quality = "strong" if rate_pct >= 60 else "moderate" if rate_pct >= 45 else "weak"
            per_type_str = (
                ", ".join(f"{k} {v:.0%}" for k, v in ft.per_type.items()) if ft.per_type else "no per-type breakdown"
            )
            cards.append(
                {
                    "t": "Breakout Follow-Through",
                    "v": (
                        f"{quality.capitalize()} continuation — {rate_pct:.0f}% of breakouts reach TP1 "
                        f"({ft.sample_count} samples). Per type: {per_type_str}."
                    ),
                }
            )
        else:
            cards.append(
                {
                    "t": "Breakout Follow-Through",
                    "v": "Insufficient breakout history — treat with caution.",
                }
            )

        # ── Trend / mean-reversion tendency ──────────────────────────────
        hurst = self.mean_reversion_tendency
        if hurst < 0.4:
            tendency = "mean-reverting (choppy) — fades work better than breakouts"
        elif hurst > 0.6:
            tendency = "trending (momentum) — breakouts have good follow-through"
        else:
            tendency = "near-random walk — mixed regime, confirm with MTF bias"
        cards.append(
            {
                "t": "Trend Tendency",
                "v": f"Hurst proxy {hurst:.2f} — {tendency}.",
            }
        )
        ...
        cards.append(
            {
                "t": "Volume Profile",
                "v": f"{self.volume_profile_shape.value.capitalize()} intraday volume distribution.",
            }
        )

        # ── Overnight gaps ────────────────────────────────────────────────
        og = self.overnight_gap
        if og.gap_frequency > 0:
            gap_bias = "gap-fill bias" if og.gap_fill_rate > og.gap_continuation_rate else "gap-continuation bias"
            cards.append(
                {
                    "t": "Overnight Gaps",
                    "v": (
                        f"Gaps on {og.gap_frequency:.0%} of sessions — {gap_bias} "
                        f"(fill {og.gap_fill_rate:.0%} / continue {og.gap_continuation_rate:.0%})."
                    ),
                }
            )
