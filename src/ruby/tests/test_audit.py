...
def tmp_db(tmp_path):
    ...
    from lib.core import db_layer, models

    # Save originals
    orig_db_path = models.DB_PATH
    orig_use_pg = models._USE_POSTGRES
    orig_layer_db_path = db_layer.DB_PATH
    orig_layer_use_pg = db_layer._USE_POSTGRES
    orig_layer_engine = db_layer._sa_engine
    orig_env_db_path = os.environ.get("DB_PATH")
    orig_env_db_url = os.environ.get("DATABASE_URL")

    # Set temp DB — must update BOTH the re-export namespace AND db_layer itself,
    # because _get_sqlite_conn() reads os.getenv("DB_PATH", db_layer.DB_PATH).
    os.environ["DB_PATH"] = db_path
    os.environ["DATABASE_URL"] = ""
    models.DB_PATH = db_path
    models._USE_POSTGRES = False
    db_layer.DB_PATH = db_path
    db_layer._USE_POSTGRES = False
    db_layer._sa_engine = None
    ...
    # Restore
    if orig_env_db_path is None:
        os.environ.pop("DB_PATH", None)
    else:
        os.environ["DB_PATH"] = orig_env_db_path
    if orig_env_db_url is None:
        os.environ.pop("DATABASE_URL", None)
    else:
        os.environ["DATABASE_URL"] = orig_env_db_url

    models.DB_PATH = orig_db_path
    models._USE_POSTGRES = orig_use_pg
    db_layer.DB_PATH = orig_layer_db_path
    db_layer._USE_POSTGRES = orig_layer_use_pg
    db_layer._sa_engine = orig_layer_engine
...
class TestEngineAuditPersistence:
    ...
    def test_record_risk_event_handles_db_error_gracefully(self, tmp_db):
        ...
        # Break the DB path to simulate an error — must update BOTH the models
        # re-export namespace AND db_layer itself AND the env var, because
        # _get_sqlite_conn() reads os.getenv("DB_PATH", db_layer.DB_PATH).
        bad_path = "/nonexistent/path/db.sqlite"
        original_env = os.environ.get("DB_PATH")
        original = models.DB_PATH
        original_layer = db_layer.DB_PATH

        os.environ["DB_PATH"] = bad_path
        models.DB_PATH = bad_path
        models._USE_POSTGRES = False
        db_layer.DB_PATH = bad_path
        db_layer._USE_POSTGRES = False
        ...
        # Restore so tmp_db teardown starts from a sane state
        if original_env is None:
            os.environ.pop("DB_PATH", None)
        else:
            os.environ["DB_PATH"] = original_env
        models.DB_PATH = original
        db_layer.DB_PATH = original_layer
