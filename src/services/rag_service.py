...
try:
    ...
    print("⚠ LangChain not available. Install with: pip install langchain langchain-community langchain-openai")

from database import Session, Document, DocumentChunk, QueryHistory, TradingInsight
...
class FKSEmbeddingsAdapter(Embeddings):
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        return [self.embeddings_service.generate_embedding(text).tolist() for text in texts]
...
Provide a detailed, professional answer:"""
    ...
    def query_with_cosine_similarity(self,
                                     query: str,
                                     top_k: int = 5,
    ...
    def augment_prompt_with_context(self,
                                    query: str,
                                    top_k: int = 5,
                                    filters: Optional[Dict[str, Any]] = None) -> Tuple[str, List[Dict[str, Any]]]:
        """
    ...
    def predict_trend(self,
        ...
        result = self.query_with_rag(
            query=query,
            top_k=10,  # More context for predictions
            filters=filters
        )
        
        # Extract prediction confidence from sources
        sources = result.get('sources', [])
        avg_similarity = np.mean([s.get('similarity_score', 0) for s in sources]) if sources else 0
...
"""
    ...
    def store_backtest_feedback(self,
                               strategy: str,
                               symbol: str,
                               timeframe: str,
