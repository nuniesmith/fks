...
Primary LLM backend: RustAssistant proxy (RUSTCODE_BASE_URL / RUSTCODE_API_KEY).
  - OpenAI-compatible ``/v1/chat/completions`` endpoint.
  - Optionally injects ``x-repo-id: <RUSTCODE_REPO_ID>`` header for RAG context.
...
---------------------
  RUSTCODE_BASE_URL      Base URL of your RustAssistant instance, e.g.
                   ``http://oryx:3500`` (no trailing slash).
  RUSTCODE_API_KEY       API key sent as ``Authorization: Bearer <key>``
                   (must match RUSTCODE_PROXY_API_KEYS on the server).
  RUSTCODE_REPO_ID       Optional repository/RAG context ID to inject.
...
def _make_ra_client():
    ...
    base_url = os.environ.get("RUSTCODE_BASE_URL", "").rstrip("/")
    api_key = os.environ.get("RUSTCODE_API_KEY", "")
    repo_id = os.environ.get("RUSTCODE_REPO_ID", "")
...
def _ra_available() -> bool:
    """True when RUSTCODE_BASE_URL and RUSTCODE_API_KEY are both set in the environment."""
    return bool(os.environ.get("RUSTCODE_BASE_URL", "").strip() and os.environ.get("RUSTCODE_API_KEY", "").strip())
...
def _call_llm(
    ...
    Attempt order:
        1. RustAssistant proxy  (RUSTCODE_BASE_URL / RUSTCODE_API_KEY)
...
class _RaClientShim:
    ...
    def base_url(self) -> str:
        return os.environ.get("RUSTCODE_BASE_URL", "").rstrip("/")

    @property
    def api_key(self) -> str:
        return os.environ.get("RUSTCODE_API_KEY", "")

    @property
    def repo_id(self) -> str:
        return os.environ.get("RUSTCODE_REPO_ID", "")
...
def format_market_context(
    ...
    from core.cache import get_cached_optimization
