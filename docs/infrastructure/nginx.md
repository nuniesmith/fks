...
- HTTPS reverse proxy to Kotlin Multiplatform Web App
- SSL/TLS encryption
...
|------|---------|-------------|
| `/` | web | Kotlin Multiplatform Web App (served via nginx) |
| `/api/` | janus | REST API endpoints (FastAPI) |
| `/static/` | Nginx | Static files from web app (cached 30 days) |
| `/health` | web | Health check endpoint |
...
**Problem:** Nginx can't reach web service  
**Solution:**
```bash
...
**Solution:**
```bash
# Check if web app is built and files are in dist directory
ls -la src/services/web/dist/

# Check volume mounts in nginx container
docker compose exec nginx ls -la /usr/share/nginx/html/

# Rebuild web app if needed
cd src/services/web && docker compose build
```
...
- [ ] Web app built and static files available
- [ ] Rate limits tested and configured
