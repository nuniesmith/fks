...
from model._shims import DEFAULT_DEVICE, log_execution, logger
...
class TFTModel(BaseModel):
    ...
    @log_execution
    def set_params(self, **params: Any) -> "TFTModel":
        """
        ...
        # Update metadata via BaseModel's _metadata (ModelMetadata object)
        if hasattr(self, "_metadata"):
            self._metadata.params.update(params)
            self._metadata.updated_at = pd.Timestamp.now().isoformat()
