...
class AssetConfig:
    ...
    maker_fee: float = 0.000025  # KuCoin Futures maker rebate (negative for discount)
    taker_fee: float = 0.0006  # KuCoin Futures taker fee
...
class RiskEngineConfig:
    """Configuration for the dynamic risk engine (leverage + margin scaling).

    All ``*_curve`` fields are piecewise-linear breakpoint tables expressed
    as ``[[x, y], ...]`` lists (sorted ascending by x).  Leave any field
    empty / omitted to use the built-in defaults.

    Quick reference — what each setting does
    -----------------------------------------
    min_scalar                  : Hard floor on any combined scalar.
                                  0.20 → leverage/margin never drops below 20 %
                                  of the configured target.
    allow_leverage_boost        : When True the engine may *raise* leverage
                                  above the asset target in calm/quality
                                  conditions (capped by max_leverage_scalar).
    max_leverage_scalar         : Upper bound on the leverage multiplier
                                  (e.g. 1.20 → max 20 % above asset.leverage).
    global_min_leverage         : Absolute leverage floor regardless of signals.
    vol_leverage_curve          : vol_pct (0-1) → leverage scalar.
    vol_margin_curve            : vol_pct (0-1) → margin scalar.
    quality_leverage_curve      : quality (0-100) → leverage scalar.
    quality_margin_curve        : quality (0-100) → margin scalar.
    drawdown_leverage_curve     : daily drawdown fraction → leverage scalar.
    drawdown_margin_curve       : daily drawdown fraction → margin scalar.
    consec_loss_scalars         : {n_losses: scalar} step table, shared for
                                  both leverage and margin.
    regime_leverage_multipliers : {regime_name: scalar} for leverage.
    regime_margin_multipliers   : {regime_name: scalar} for margin.
    """

    enabled: bool = True
    min_scalar: float = 0.20
    allow_leverage_boost: bool = True
    max_leverage_scalar: float = 1.20
    global_min_leverage: int = 2

    # Volatility curves — leave empty to use module defaults
    vol_leverage_curve: list = field(default_factory=list)
    vol_margin_curve: list = field(default_factory=list)

    # Quality curves
    quality_leverage_curve: list = field(default_factory=list)
    quality_margin_curve: list = field(default_factory=list)

    # Drawdown curves
    drawdown_leverage_curve: list = field(default_factory=list)
    drawdown_margin_curve: list = field(default_factory=list)

    # Consecutive loss step table
    consec_loss_scalars: dict = field(
        default_factory=lambda: {0: 1.0, 1: 0.90, 2: 0.75, 3: 0.60, 4: 0.45}
    )

    # Regime multipliers
    regime_leverage_multipliers: dict = field(
    ...
    regime_margin_multipliers: dict = field(
        default_factory=lambda: {
            "trending": 1.05,
            "neutral": 1.00,
            "ranging": 0.85,
...
def _build_risk_engine(raw: dict | None) -> RiskEngineConfig:
    if raw is None:
        return RiskEngineConfig()

    def _float_pairs(val: Any) -> list:
        """Parse [[x, y], ...] from config; return empty list on failure."""
        if not val:
            return []
        try:
    ...
    consec_raw = raw.get("consec_loss_scalars") or {}
    consec = {int(k): _safe_float(v, 1.0) for k, v in consec_raw.items()} if consec_raw else {}

    reg_lev_raw = raw.get("regime_leverage_multipliers") or {}
    reg_lev = {str(k).lower(): _safe_float(v, 1.0) for k, v in reg_lev_raw.items()}

    reg_marg_raw = raw.get("regime_margin_multipliers") or {}
    reg_marg = {str(k).lower(): _safe_float(v, 1.0) for k, v in reg_marg_raw.items()}

    return RiskEngineConfig(
        enabled=_safe_bool(raw.get("enabled"), True),
        min_scalar=_safe_float(raw.get("min_scalar"), 0.20),
        allow_leverage_boost=_safe_bool(raw.get("allow_leverage_boost"), True),
        max_leverage_scalar=_safe_float(raw.get("max_leverage_scalar"), 1.20),
        global_min_leverage=_safe_int(raw.get("global_min_leverage"), 2),
        vol_leverage_curve=_float_pairs(raw.get("vol_leverage_curve")),
        vol_margin_curve=_float_pairs(raw.get("vol_margin_curve")),
        quality_leverage_curve=_float_pairs(raw.get("quality_leverage_curve")),
        quality_margin_curve=_float_pairs(raw.get("quality_margin_curve")),
        drawdown_leverage_curve=_float_pairs(raw.get("drawdown_leverage_curve")),
        drawdown_margin_curve=_float_pairs(raw.get("drawdown_margin_curve")),
        consec_loss_scalars=consec
        or RiskEngineConfig.__dataclass_fields__["consec_loss_scalars"].default_factory(),
        regime_leverage_multipliers=reg_lev
        or RiskEngineConfig.__dataclass_fields__["regime_leverage_multipliers"].default_factory(),
        regime_margin_multipliers=reg_marg
        or RiskEngineConfig.__dataclass_fields__["regime_margin_multipliers"].default_factory(),
    )
...
def load_config(path: str | None = None) -> FuturesConfig:
    ...
    cfg = FuturesConfig(
        ...
        risk_engine=_build_risk_engine(data.get("risk_engine")),
