...
Strategies Package
==================
Trading strategy modules for Ruby.
...
the CNN feature vector (v7+) and the dashboard daily plan view::
    ...
    from trading.strategies.daily import (
        compute_daily_bias,
        DailyBias,
        BiasDirection,
    )
...
Phase 4B additions (CNN Sub-Feature Decomposition):
  ...
  These sub-features are computed in ``analysis.breakout_cnn`` and wired
  into ``dataset_generator._build_row()`` and ``feature_contract.json`` v7.1.
"""

from trading.strategies.strategy_defs import (
    STRATEGY_CLASSES,
    STRATEGY_LABELS,
    BreakoutStrategy,
    EventReaction,
