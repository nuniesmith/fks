...
class CoinMarketCapClient(BaseClient):
    ...
    BASE_URL = "https://pro-api.coinmarketcap.com/v1/fkscurrency/quotes/latest"
    LISTINGS_LATEST_URL = "https://pro-api.coinmarketcap.com/v1/fkscurrency/listings/latest"  # For health check
    ...
    def fetch_data(self, symbol: str) -> Optional[pd.DataFrame]:
        ...
        Args:
            symbol (str): The fkscurrency symbol (e.g., "BTC").
...
@router.get("/data", summary="Fetch real-time fkscurrency data from CoinMarketCap")
def fetch_cmc_data(
    symbol: str = Query(..., description="Cryptocurrency symbol (e.g., BTC)")
):
    """
    Endpoint to fetch real-time fkscurrency data for a given symbol.
    """
