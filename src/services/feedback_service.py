...
from database import Document, Session, TradingInsight
...
"""
        ...
        self._store_trading_insight(
            ...
            metadata={"strategy": strategy, "outcome": outcome, "pnl_pct": pnl_pct},
        )

        print(
            f"✓ Trade outcome logged: {symbol} {outcome} ({pnl_pct:+.2f}%) - Doc ID: {doc_id}"
        )
        return doc_id
...
"""
        ...
        print(
            f"✓ Backtest logged: {strategy} on {symbol} - {performance} - Doc ID: {doc_id}"
        )
        return doc_id
    ...
    def analyze_strategy_performance(
        ...
        result = self.rag.query_with_rag(
            query=query, top_k=15, filters=filters, session=session
        )
    ...
    def get_best_strategy_for_condition(
        self, symbol: str, market_condition: str, lookback_days: int = 60
    ) -> dict[str, Any]:
        """
        ...
        result = self.rag.query_with_rag(query=query, top_k=10, filters=filters)
    ...
    def learn_from_losses(
        ...
        """
        query_base = "Analyze the losing trades"
        if symbol:
            query_base += f" for {symbol}"
        query = (
            query_base
            + f" in the last {lookback_days} days. What patterns led to losses and how can they be avoided?"
        )
    ...
    def get_optimization_suggestions(
        self, strategy: str, symbol: str, current_params: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        result = self.rag.query_with_rag(query=query, top_k=12, filters=filters)
