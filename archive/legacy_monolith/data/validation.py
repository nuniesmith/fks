...
def _detect_timeframe_seconds(df: pd.DataFrame, dt_col: str) -> int | None:
    if df.empty:
        return None
...
def validate_ohlcv(df: pd.DataFrame) -> QualityReport:
    ...
    missing_pct = (
        float(100.0 * missing / (rows * max(1, len(numeric_cols)))) if rows else 0.0
    )
...
def detect_anomalies(df: pd.DataFrame, z_threshold: float = 5.0) -> pd.DataFrame:
    ...
    if all(c in work.columns for c in REQUIRED_COLS):
        invalid = (work["high"] < work[["open", "close"]].max(axis=1)) | (
...
def cross_validate(
    a: pd.DataFrame, b: pd.DataFrame, tolerance: float = 0.01
) -> dict[str, Any]:
    ...
    try:
        ...
        if (
            merged.empty
            or "close_a" not in merged.columns
            or "close_b" not in merged.columns
        ):
            return {"aligned": 0, "mismatch_pct": None, "sample": []}
        rel_diff = (merged["close_a"] - merged["close_b"]).abs() / merged[
            ["close_a", "close_b"]
        ].mean(axis=1)
...
def compute_time_splits(
    df: pd.DataFrame, train: float = 0.8, val: float = 0.1, test: float = 0.1
