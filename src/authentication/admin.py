...
from .models import User, UserProfile, APIKey, UserSession


@admin.register(User)
class UserAdmin(BaseUserAdmin):
    """Admin interface for User model"""
    list_display = ('username', 'email', 'user_type', 'status', 'last_active', 'is_staff')
    list_filter = ('user_type', 'status', 'is_staff', 'is_active')
    search_fields = ('username', 'email', 'first_name', 'last_name')
    
    fieldsets = BaseUserAdmin.fieldsets + (
        ('User Type & Status', {
            'fields': ('user_type', 'status', 'api_key_enabled')
        }),
        ('Session Management', {
            'fields': ('max_concurrent_sessions',)
        }),
        ('Additional Info', {
...
class UserProfileAdmin(admin.ModelAdmin):
    ...
    list_display = ('user', 'risk_tolerance', 'total_trades', 'success_rate', 'created_at')
    ...
    fieldsets = (
        ...
        ('Notifications', {
            'fields': ('email_notifications', 'discord_notifications', 'discord_webhook')
        }),
        ('API Credentials', {
        ...
        ('Statistics', {
            'fields': ('total_trades', 'successful_trades'),
            'classes': ('collapse',)
        }),
    )
...
class APIKeyAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'user', 'is_active', 'last_used', 'rate_limit', 'created_at')
