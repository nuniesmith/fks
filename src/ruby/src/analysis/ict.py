"""Adapted from ruby/lib/analysis/ict.py — ICT / Smart Money Concepts."""
...
def _normalize_columns(df):
    """Accept both lower and upper case OHLCV columns."""
    mapping = {}
    for col in ["open", "high", "low", "close", "volume"]:
        if col in df.columns and col[0].upper() + col[1:] not in df.columns:
            mapping[col] = col[0].upper() + col[1:]
    if mapping:
...
def detect_fvgs(
    ...
    Returns:
        List of dicts, each containing:
          ...
          - top / bottom / midpoint / size / size_atr
          - bar_index, timestamp
          - filled (bool), fill_pct (0.0–1.0)
    """
    ...
    for i in range(2, len(df)):
        ...
        l1 = float(low.iloc[i - 2])  # candle 1 low
