...
from model.utils.metadata import ModelMetadata
...
class EnsembleModel(BaseModel):
    ...
    def fit(self, train_data: Any, target_column: Any = None, **kwargs: Any) -> "EnsembleModel":
        """Fit the ensemble by training each base model."""
        self.train(train_data, target_column=target_column, **kwargs)
        return self

    def generate_samples(self, n_samples: int = 10, **kwargs: Any) -> pd.DataFrame:
        """Generate samples by aggregating from base models."""
        models_with_generation = [m for m in self.models if hasattr(m, "generate_samples")]
        ...
        for model in models_with_generation:
            try:
                samples = model.generate_samples(n_samples=max(1, n_samples // len(models_with_generation)), **kwargs)
