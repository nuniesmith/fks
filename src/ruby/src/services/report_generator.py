...
=======================================================================
Uses xAI API (Grok 4.2 (grok-4.20-0309-reasoning)) to generate intelligent trading performance
reports from Redis trade data.
...
def _get_tracked_assets() -> list[str]:
    """Return uppercase base names of all registered assets."""
    return [spec.base for spec in ASSET_REGISTRY.values()]


_SYSTEM_BASE = (
    "You are a senior quantitative crypto-futures trading performance analyst. "
    "You work for a systematic trading desk that runs a multi-asset perpetual futures "
    "scalping bot. The bot trades across multiple assets on cryptocurrency futures "
    "exchanges.\n\n"
...
class ReportGenerator:
    ...
    def __init__(
        ...
        Args:
            ...
            discord: Optional DiscordNotifier for posting report summaries.
        """
    ...
    async def _notify_discord(self, report_type: str, report: str) -> None:
        """Post report summary to Discord if notifier is configured."""
        if self._discord is None:
            return
        try:
            # Send first ~2000 chars as a summary
            summary = report[:2000]
            await self._discord.grok_report(report_type, summary)
        except Exception:
            logger.debug("Discord notification for %s report failed", report_type)
    ...
    async def generate_daily_report(self) -> str | None:
        ...
        try:
            ...
            if report:
                ...
                await self._notify_discord("daily", report)
            return report
        except Exception:
    ...
    async def generate_weekly_report(self) -> str | None:
        ...
        try:
            ...
            if report:
                ...
                await self._notify_discord("weekly", report)
            return report
        except Exception:
    ...
    async def generate_monthly_report(self) -> str | None:
        ...
        try:
            ...
            if report:
                ...
                await self._notify_discord("monthly", report)
            return report
        except Exception:
    ...
    async def generate_highlights(self) -> str | None:
        ...
        try:
            ...
            if report:
                ...
                await self._notify_discord("highlights", report)
            return report
        except Exception:
    ...
    async def _call_grok(
        ...
        """Call Grok 4.2 (grok-4.20-0309-reasoning) via xAI API using the ``openai`` SDK.
    ...
    def _build_weekly_prompt(self, data: dict, log_context: str) -> tuple[str, str]:
        ...
        system_prompt = (
            ...
            "2. **🏅 Asset Performance Ranking** — table ranking all tracked assets by PnL, "
    ...
    def _build_monthly_prompt(self, data: dict, log_context: str) -> tuple[str, str]:
        ...
        system_prompt = (
            ...
            "3. **🏅 Asset Allocation Effectiveness** — rank all tracked assets by PnL. "
